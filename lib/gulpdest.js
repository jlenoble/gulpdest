...
var SimpleGulpDest = exports.SimpleGulpDest = function () {
  ...
  _createClass(SimpleGulpDest, [{
    ...
    value: function dest(stream) {
      ...
      var destGlb = glb.dest(this.destination, {
        ready: function ready() {
          return new Promise(function (resolve, reject) {
            stream.pipe(_gulp2.default.dest(_this.destination)).on('error', reject).on('end', resolve);
          }).then(function () {
            return Promise.all(destGlb.paths.map(function (path) {
              return new Promise(function (resolve, reject) {
                (0, _glob2.default)(path, function (err, files) {
                  if (err) {
                    return reject(err);
                  }
                  return Promise.all(files.map(function (file) {
                    return new Promise(function (resolve, reject) {
                      var date = new Date();
                      _fs2.default.utimes(file, date, date, function (err) {
                        if (err) {
                          return reject(err);
                        }
                        resolve();
